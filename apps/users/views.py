...
from organzation.models import CourseOrg,Teacher
...
#它有authenticate方法用邮箱登录时，能够扩展 默认的 User 模型，或实现 一个完全定制的模型。
class CoustomBackend(ModelBackend):
    def authenticate(self,request,username=None,password=None,**kwargs):
        try:
            # 不希望用户存在两个，get异能有一个。两个是get失败的一种原因 Q为使用并集查询
            user = UserProfile.objects.get(Q(username=username)|Q(email=username))

            # django的后台中密码加密：所以不能password=password
            # UserProfile继承的AbstrctUser中的def check_password(self,raw_password）
