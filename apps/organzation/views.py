...
class OrgView(View):
    ...
    def get(self, request):
        ...
        # 如果是不合法的配置参数默认返回第一页
        #先获取需要展示的全部数据，然后使用 Paginator 类对数据进行分页，最后返回每个页面的 page 对象。
        # page 对象的作用巨大，一方面展示当前分页数据，还提供获取后续页面数据的接口。
        try:
        ...
        # 使用 paginator 对象返回每页的page对象
        # 在前端页面中显示数据，我们主要的操作都是基于Page对象
        orgs = p.page(page)
